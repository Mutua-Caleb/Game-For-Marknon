function soromathEmbeddedInHost(){
  try{
    return window.self != window.top && new URLSearchParams(window.location.search).has("inside");
  }
  catch(error){
    return true;
  }
...
function restarttest(focus=true){
  ...
  if(typeof activeStudyReset == "function") activeStudyReset();

  if(focus && !soromathEmbeddedInHost()) window.setTimeout( () => input.focus(), 50 );
...
function starttest(){
  ...
  teststarted = 0;
  if(typeof activeStudyStart == "function") activeStudyStart();
